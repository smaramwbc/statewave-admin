...
describe('dispatch — inventory', () => {
  it('exposes the full set of public admin routes', () => {
    ...
    expect(Object.values(ROUTES).sort()).toEqual(
      [
        ...
        '/api/admin-settings',
        '/api/admin-settings/confirm',
        '/api/admin-settings/enable-admin-auth',
        '/api/admin-settings/revert',
        '/api/admin/proxy-info',
        '/api/admin/readiness-check',
        '/api/proxy',
  ...
  it('returns true for every known route (so the catch-all forwards them)', async () => {
    ...
    const knownPaths = [
      ...
      ROUTES.adminSettingsConfirm,
      ROUTES.adminSettingsRevert,
